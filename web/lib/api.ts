...
export async function authenticate(initData: string): Promise<Auth> {
  return parse(await fetch(`${backendUrl}/api/auth`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ init_data: initData }) }));
}

export function client(token: string) {
  const request = async (path: string, init: RequestInit = {}) => parse(await fetch(`${backendUrl}${path}`, {
    ...init,
  ...
  return {
    ...
    compassPreferences: (payload: Record<string, unknown>) => request("/api/compass/preferences", { method: "POST", body: JSON.stringify(payload) }),
...
export function roomCandidate(startParam?: string) {
