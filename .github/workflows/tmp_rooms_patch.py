...
r('if not (privileged or moderator) or target == row["owner_id"] or protected_moderator:','if (\n                not can_shared\n                or (row["kind"] == "personal" and target == row["owner_id"])\n                or protected_moderator\n            ):')
r(b(I*3+'if not personal_owner:',I*4+'raise RoomError("owner_required", "Only the owner can approve re-entry", 403)'),b(I*3+'if not privileged:',I*4+'raise RoomError(',I*5+'"owner_required", "Only the room authority can approve re-entry", 403',I*4+')'))
