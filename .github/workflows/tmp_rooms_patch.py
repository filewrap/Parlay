...
replace(block(I*2+'self.on_action = on_action', I*2+'self._locks: dict[str, asyncio.Lock] = {}'), block(I*2+'self.on_action = on_action', I*2+'self.playback_timeout = 90.0', I*2+'self._locks: dict[str, asyncio.Lock] = {}'))
replace(block(I*2+'room_id = await asyncio.to_thread(ensure)', I*2+'result = await self.snapshot(room_id, owner_id)', I*2+'self._publish(room_id, result)', I*2+'return result'), block(I*2+'room_id = await asyncio.to_thread(ensure)', I*2+'if not await self._check(self.member, owner_id, chat_id):', I*3+'raise RoomError(', I*4+'"not_group_member", "Telegram group membership is required", 403', I*3+')', I*2+'async with self._lock(room_id):', I*3+'row = await asyncio.to_thread(self._load, room_id)', I*3+'self._active(row)', I*3+'data = self._data(row)', I*3+'admitted = any(item["user_id"] == owner_id for item in data["members"])', I*3+'revision = row["revision"]', I*3+'if not admitted:', I*4+'if len(data["members"]) >= data["settings"]["capacity"]:', I*5+'raise RoomError("room_full", "The room is at capacity", 409)', I*4+'data["members"].append(', I*5+'{"user_id": owner_id, "first_name": str(owner_id), "role": "participant"}', I*4+')', I*4+'revision += 1', I*4+'await asyncio.to_thread(self._save, room_id, revision, data)', I*3+'result = await self._snapshot_for(', I*4+'self._replace(row, revision, data), owner_id', I*3+')', I*2+'if not admitted:', I*3+'self._publish(room_id, result)', I*2+'return result'))
needle=block(I*3+'row = await asyncio.to_thread(self._load, room_id)', I*3+'self._active(row)', I*3+'data = self._data(row)', I*3+'member = next((m for m in data["members"] if m["user_id"] == user_id), None)')
replace(needle, block(I*3+'row = await asyncio.to_thread(self._load, room_id)', I*3+'self._active(row)', I*3+'self._actionable(row)', I*3+'data = self._data(row)', I*3+'member = next((m for m in data["members"] if m["user_id"] == user_id), None)'))
replace(block(I*4+'actual = await self.playback(*playback_call)', I*4+'if not actual:', I*5+'raise RoomError(', I*6+'"playback_unavailable", "Group playback did not confirm the action", 503', I*5+')', I*4+'data["playback"] = self._clean_playback(actual)'), block(I*4+'try:', I*5+'actual = await asyncio.wait_for(', I*6+'self.playback(*playback_call), timeout=self.playback_timeout', I*5+')', I*4+'except TimeoutError as exc:', I*5+'raise RoomError(', I*6+'"playback_timeout", "Group playback timed out. Retry the action.", 504', I*5+') from exc', I*4+'if not actual:', I*5+'raise RoomError(', I*6+'"playback_unavailable", "Group playback did not confirm the action", 503', I*5+')', I*4+'row = await asyncio.to_thread(self._load, room_id)', I*4+'self._active(row)', I*4+'self._actionable(row)', I*4+'data = self._data(row)', I*4+'data["playback"] = self._clean_playback(actual)'))
replace(block(I*2+'privileged = authority if row["kind"] == "group" else personal_owner', I*2+'notify = playback_call = None'), block(I*2+'privileged = authority if row["kind"] == "group" else personal_owner', I*2+'delegated = moderator and row["kind"] == "personal"', I*2+'can_shared = privileged or delegated', I*2+'notify = playback_call = None'))
replace('if not (privileged or moderator or settings["queue_all"]):','if not (can_shared or settings["queue_all"]):')
replace(block(I*3+'if not (',I*4+'privileged',I*4+'or moderator',I*4+'or (settings["queue_all"] and not settings["owner_lock"])',I*3+'):') , block(I*3+'if not (',I*4+'can_shared',I*4+'or (',I*5+'row["kind"] == "personal"',I*5+'and settings["queue_all"]',I*5+'and not settings["owner_lock"]',I*4+')',I*3+'):'))
replace('if not (privileged or moderator) or target == row["owner_id"] or protected_moderator:', 'if (\n'+I*4+'not can_shared\n'+I*4+'or (row["kind"] == "personal" and target == row["owner_id"])\n'+I*4+'or protected_moderator\n'+I*3+'):')
replace(block(I*3+'if not personal_owner:',I*4+'raise RoomError("owner_required", "Only the owner can approve re-entry", 403)'), block(I*3+'if not privileged:',I*4+'raise RoomError(',I*5+'"owner_required", "Only the room authority can approve re-entry", 403',I*4+')'))
replace(block(I*2+'can_shared = authority or role == "moderator"',I*2+'if row["kind"] == "personal" and settings["owner_lock"] and not personal_owner:',I*3+'can_shared = False'), block(I*2+'delegated = role == "moderator" and row["kind"] == "personal"',I*2+'can_shared = authority or delegated',I*2+'if row["kind"] == "personal" and settings["owner_lock"] and not personal_owner:',I*3+'can_shared = False'))
replace(block(I*4+'"control": can_shared',I*4+'or (settings["queue_all"] and not settings["owner_lock"]),'), block(I*4+'"control": can_shared',I*4+'or (',I*5+'row["kind"] == "personal"',I*5+'and settings["queue_all"]',I*5+'and not settings["owner_lock"]',I*4+'),'))
replace(I+'def _active(self, row):\n', block(I+'@staticmethod',I+'def _actionable(row) -> None:',I*2+'if row["state"] == "recovering":',I*3+'raise RoomError(',I*4+'"room_recovering",',I*4+'"Room playback is recovering. Refresh and retry.",',I*4+'409,',I*3+')','',I+'def _active(self, row):'))
p.write_text(s)

t=Path('tests/test_rooms_service.py')
tests=t.read_text()
assert 'test_ensure_group_admits_later_live_operator' not in tests
tests += r'''
